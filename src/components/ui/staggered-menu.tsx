...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (!panel || !top || !bot || !mid || !icon) return;
      ...
      gsap.set(top, { transformOrigin: '50% 50%', y: -6, rotate: 0 });
      gsap.set(mid, { transformOrigin: '50% 50%', y: 0, opacity: 1 });
      gsap.set(bot, { transformOrigin: '50% 50%', y: 6, rotate: 0 });
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {/* Logo removed to prevent duplicating the main Navbar logo */}

          <button
            ref={toggleBtnRef}
            className={`sm-toggle relative inline-flex items-center justify-center w-10 h-10 bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto ml-auto ${
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
            ...
            <div className="mt-8">
              <button 
                onClick={closeMenu}
                className="w-full py-4 rounded-xl border-2 border-black/10 text-black font-semibold uppercase tracking-wider hover:bg-black/5 transition-colors flex items-center justify-center gap-2"
              >
                Close Menu
              </button>
            </div>
